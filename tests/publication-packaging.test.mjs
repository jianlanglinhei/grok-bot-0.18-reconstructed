...
test("default packaging keeps the polished checksum-pinned renderer", async () => {
  ...
  assert.match(source, /const helperSuffixes = \["", " \(GPU\)", " \(Plugin\)", " \(Renderer\)"\]/);
  assert.match(source, /\["-replace", "CFBundleExecutable", "-string", reconstructedName, infoPlist\]/);
});
...
test("Router settings use the trusted backend and display recorded inference usage", async () => {
  ...
  const codexCli = await readFile(path.join(repoRoot, "source", "host", "extensions", "inference", "codex-cli.ts"), "utf8");
  ...
  const coordinatorExecutors = await readFile(path.join(repoRoot, "source", "electron-main", "coordinator", "coordinator-executors.ts"), "utf8");
  ...
  const aoneSandbox = await readFile(path.join(repoRoot, "source", "electron-main", "box", "aone-sandbox-host-connector.ts"), "utf8");
  const productionBox = await readFile(path.join(repoRoot, "source", "host", "box", "production.ts"), "utf8");
  const foreverBoxExtension = await readFile(path.join(repoRoot, "source", "host", "extensions", "forever-box", "extension.ts"), "utf8");
  const hostRunnerComposition = await readFile(path.join(repoRoot, "source", "host", "host-runner-composition.ts"), "utf8");
  ...
  assert.match(rendererPatch, /desktop\.agent\.setBoxRuntime\(d\)/);
  assert.match(rendererPatch, /role:"switch"/);
  assert.match(rendererPatch, /Use local Docker VM/);
  assert.match(rendererPatch, /Use local Codex CLI/);
  assert.match(rendererPatch, /Use Aone Sandbox/);
  assert.match(rendererPatch, /desktop\.agent\.saveAoneSandboxApiKey/);
  assert.match(rendererPatch, /onValueChange:[a-z]=>\{if\([a-z]!==null\)void e\([a-z]\)\}/);
  ...
  assert.match(preload, /setLocalCodexCliEnabled: \(enabled: boolean\) => edge\("setLocalCodexCliEnabled", \{ enabled \}\)/);
  ...
  assert.match(preload, /getAoneSandboxConfig: \(\) => edge\("getAoneSandboxConfig"\)/);
  ...
  assert.match(mainEdge, /invoke\(deps\.settingsStore, "setUseLocalCodexCli", enabled\)/);
  assert.match(mainEdge, /return \{ provider, usage:/);
  assert.match(mainEdge, /invoke\(deps\.boxRecovery, "restartCoordinator"\)/);
  assert.match(mainEdge, /mode === "aone-sandbox" && !await isAoneSandboxConfigured\(\)/);
  assert.match(mainEdge, /mode === "aone-sandbox" \? await getAoneSandboxStatus\(settingsPath\)/);
  ...
  assert.match(localDocker, /runtime === "aone-sandbox"\) return await aone\.connect\(\)/);
  assert.match(aoneSandbox, /@ali\/aone-sandbox/);
  assert.match(aoneSandbox, /readA1GroundApiKey/);
  assert.match(aoneSandbox, /hub\.docker\.alibaba-inc\.com\/aone-base-global\/code-interpreter/);
  assert.match(aoneSandbox, /dynamicTemplate: \{ image: AONE_SANDBOX_IMAGE/);
  assert.match(aoneSandbox, /\/root\/\.codex\/auth\.json/);
  assert.match(aoneSandbox, /Log in with a1 ground or save a key/);
  assert.match(aoneSandbox, /\/opt\/onebot\/host\/host-main\.cjs/);
  assert.match(aoneSandbox, /supportFiles/);
  assert.match(aoneSandbox, /\/opt\/onebot\/host\/\$\{file\.relativePath\}/);
  assert.match(aoneSandbox, /inferenceProvider: args\.provider/);
  assert.match(aoneSandbox, /active\.state\.provider === provider/);
  assert.match(aoneSandbox, /killPersistedSandbox\(state, key, "runtime changed"\)/);
  assert.match(aoneSandbox, /listRoutedComputerTools/);
  assert.match(aoneSandbox, /executeRoutedComputerTool/);
  assert.match(aoneSandbox, /SAND_CODEX_RESPONSES_ENDPOINT/);
  assert.match(aoneSandbox, /startLocalCodexRelay/);
  assert.match(aoneSandbox, /\/opt\/onebot\/codex-relay\.cjs/);
  assert.match(aoneSandbox, /AONE_SANDBOX_VNC_PORT/);
  assert.match(aoneSandbox, /buildAoneNoVncUrl/);
  assert.match(aoneSandbox, /xorg-x11-server-Xvfb/);
  assert.match(aoneSandbox, /--token-plugin TokenFile/);
  assert.match(aoneSandbox, /\/opt\/onebot\/box-exec-daemon\/main\.cjs/);
  assert.match(aoneSandbox, /SAND_SINGLE_DESKTOP_VNC: "1"/);
  assert.match(aoneSandbox, /ONEBOT_BOX_RUNTIME: "aone-sandbox"/);
  assert.match(productionBox, /vncUrl: exposePrimaryVnc \? primary\.vncUrl : ""/);
  assert.match(productionBox, /exposePrimaryVnc\s*\? primary\.remoteAccessor\s*:\s*withNoMonitorComputerUse/);
  assert.match(foreverBoxExtension, /standaloneVnc: context\.host\.standaloneVisualDesktop === true/);
  assert.match(hostRunnerComposition, /createSandComputerUseSubagentConfig\(\{ browserUseOffered \}\)/);
  ...
  assert.match(codexCli, /"exec"/);
  assert.match(codexCli, /"--json"/);
  assert.match(codexCli, /"--ephemeral"/);
  assert.match(codexCli, /"--sandbox", "read-only"/);
  assert.match(codexCli, /mcp_servers\.onebot_plugins\.url/);
  ...
  assert.match(providers, /return "medium"/);
  ...
  assert.match(coordinator, /shouldRouteInferenceInCoordinator\(provider, settings\.getBoxRuntime\(\), settings\.getUseLocalCodexCli\(\)\)/);
  assert.match(coordinator, /provider === "codex" && settings\.getUseLocalCodexCli\(\)/);
  ...
  assert.match(coordinator, /setTimeout\(resolve, 200\)/);
  assert.match(coordinatorExecutors, /retireSupersededOwnedDaemons/);
  assert.match(coordinatorExecutors, /await native\.terminateProcess\(identity\.pid\)/);
