...
import { getAoneSandboxStatus, isAoneSandboxConfigured, saveAoneSandboxApiKey } from "./box/aone-sandbox-host-connector.js";
...
export function createMainEdgeHandlers(deps: MainEdgeDeps): HandlerMap {
  const handlers: HandlerMap = {
    ...
    getInferenceRouter: async () => { const settings = await deps.readHostSettingsFromBox().catch(() => ({} as UnknownRecord)); const provider = invoke(deps.settingsStore, "getInferenceProvider"); return { provider: isSandInferenceProvider(provider) ? provider : "cursor", usage: settings.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus(), useLocalCodexCli: invoke(deps.settingsStore, "getUseLocalCodexCli") === true }; },
    setInferenceRouter: async (raw) => { const provider = req(raw).provider; invariant(isSandInferenceProvider(provider), "Unknown inference provider."); invoke(deps.settingsStore, "setInferenceProvider", provider); const settings = await deps.syncHostSettingsToBox({ inferenceProvider: provider }).catch(() => null); return { provider, usage: settings?.inferenceRouterUsage ?? invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: getLocalInferenceCliStatus(), useLocalCodexCli: invoke(deps.settingsStore, "getUseLocalCodexCli") === true }; },
    setLocalCodexCliEnabled: (raw) => { const enabled = req(raw).enabled === true; const status = getLocalInferenceCliStatus(); if (enabled) { invariant(status.codex.installed, "Codex CLI is not installed."); invariant(status.codex.authenticated, "Codex CLI is not signed in. Run codex login first."); } invoke(deps.settingsStore, "setUseLocalCodexCli", enabled); const provider = invoke(deps.settingsStore, "getInferenceProvider"); return { provider: isSandInferenceProvider(provider) ? provider : "cursor", usage: invoke(deps.settingsStore, "getInferenceRouterUsage") ?? null, local: status, useLocalCodexCli: enabled }; },
    getAoneSandboxConfig: async () => ({ configured: await isAoneSandboxConfigured() }),
    saveAoneSandboxApiKey: async (raw) => { const value = req(raw).apiKey; invariant(typeof value === "string", "Aone Sandbox API key must be a string."); await saveAoneSandboxApiKey(value); return { configured: true }; },
    getBoxRuntime: async () => { const mode = invoke(deps.settingsStore, "getBoxRuntime"); invariant(isSandBoxRuntime(mode), "Unknown box runtime."); const settingsPath = String(Reflect.get(deps.settingsStore, "settingsPath")); return { mode, status: mode === "aone-sandbox" ? await getAoneSandboxStatus(settingsPath) : await getLocalDockerStatus(settingsPath) }; },
    setBoxRuntime: async (raw) => { const mode = req(raw).mode; invariant(isSandBoxRuntime(mode), "Unknown box runtime."); const settingsPath = String(Reflect.get(deps.settingsStore, "settingsPath")); const previous = invoke(deps.settingsStore, "getBoxRuntime"); invariant(isSandBoxRuntime(previous), "Unknown previous box runtime."); if (mode === "aone-sandbox" && !await isAoneSandboxConfigured()) throw new Error("Save an Aone Sandbox API key before switching."); invoke(deps.settingsStore, "setBoxRuntime", mode); try { if (mode === "local-docker") await startLocalDockerBox(settingsPath); else await stopLocalDockerBox(); } catch (error) { invoke(deps.settingsStore, "setBoxRuntime", previous); throw error; } invoke(deps.boxRecovery, "restartCoordinator"); return { mode, status: mode === "aone-sandbox" ? await getAoneSandboxStatus(settingsPath) : await getLocalDockerStatus(settingsPath) }; },
