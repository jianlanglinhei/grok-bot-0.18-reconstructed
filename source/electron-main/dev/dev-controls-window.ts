...
export function openDevControlsWindow(options: { readonly electron: DevControlsWindowElectronPort; readonly devServerUrl?: string; readonly preloadPath: string }): void {
  ...
  const window = options.electron.createBrowserWindow({ width, height, minWidth: 660, minHeight: 460, x: workArea.x + workArea.width - width - margin, y: workArea.y + margin, title: "onebot Dev Controls", resizable: true, minimizable: true, maximizable: true, fullscreenable: false, backgroundColor: "#0f1117", autoHideMenuBar: true, webPreferences: { contextIsolation: true, nodeIntegration: false, preload: options.preloadPath, sandbox: false } });
