...
export const SAND_NO_LOCAL_MACHINE_MESSAGE =
  "Your local machine isn't connected right now (the onebot desktop app must be open and online to run commands on it). Try again once it's reachable.";
...
export function localExecFileTooLargeMessage(actualBytes: number, maxBytes: number): string {
  return `File is ${describeLocalExecBytes(actualBytes)}, which exceeds onebot's ${describeLocalExecBytes(maxBytes)} limit for reading or transferring a single file over local-exec. Read a slice with offset/limit, or use a shell command (grep, head, tail) to extract just what you need.`;
}

export function localExecUploadFrameTooLargeMessage(maxBytes: number): string {
  return `The upload exceeds onebot's ${describeLocalExecBytes(maxBytes)} limit for transferring a single file over local-exec and was refused before being read into memory. Transfer a smaller file, or split it into parts.`;
}
