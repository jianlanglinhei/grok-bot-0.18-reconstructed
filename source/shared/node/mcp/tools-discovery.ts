    ...
    if (
      ...
      try {
        ...
        return resultFactory.error(
          `Could not load MCP servers onto onebot's computer: ${errorMessage(error)}`,
        );
      }
      return boxMcpExec.executeTool(args);
    }
    return resultFactory.error(
      `MCP server "${args.providerIdentifier}" is not available here. HTTP/SSE servers execute on the backend and stdio servers run on onebot's computer; this server is neither reachable nor supported in this context.`,
    );
  }
