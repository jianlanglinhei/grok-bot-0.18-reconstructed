...
export const automationsExtension = defineHostExtension({
  ...
  start: (context) => {
    ...
    const cloudSyncOptions: ConstructorParameters<typeof SandAutomationCloudSync>[0] & {
      ...
      onFailure: ({ agentId }) => { if (agentId == null || routineSyncFailureTrayIds.has(agentId)) return; const tray = deps.trays.pushError({ agentId, title: "Routine Sync Failed", detail: "onebot couldn't sync this agent's routines. Event routines keep running locally when safe, but scheduled routines may be delayed while onebot retries." }); routineSyncFailureTrayIds.set(agentId, tray.id); },
