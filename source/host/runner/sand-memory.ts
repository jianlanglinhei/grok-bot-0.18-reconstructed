...
export function buildEpisodeSystemPrompt(): string { return [
  ...
  "You maintain the long-term memory of a personal desktop assistant named onebot.",
  "You are given the most recent turns of a conversation between the user and onebot, in order, each tagged with its date.",
  "Write ONE short journal-style sentence (two at most) capturing what the user and onebot were actually working on across these turns \u2014 the throughline, key decisions, and outcomes \u2014 so it stays useful months from now.",
...
export function buildEpisodeUserPrompt(turns: readonly EpisodeTurn[]): string { return ["Recent turns, oldest first:", "", turns.map((turn) => [`(${formatMemoryDate(turn.ts)})`, ...(turn.user.trim() ? [`User: ${turn.user.trim()}`] : []), ...(turn.agent.trim() ? [`onebot: ${turn.agent.trim()}`] : [])].join("\n")).join("\n\n")].join("\n"); }
