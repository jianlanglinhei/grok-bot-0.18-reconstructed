  ...
  while (!isCancelled()) {
    try {
      ...
      if (error instanceof SandLocalToolPermissionDeniedError) {
        settle(
          "error",
          "onebot is no longer allowed to read this command's output on the user's computer, so its completion cannot be observed. The command keeps running; ask the user to approve reading its output file for the result.",
          outputPath,
        );
        return;
      }
