...
export function summarizeBlockedAction(target: ReturnType<typeof buildSandComputerAutoReviewCanonicalTarget>, fingerprint: string, reason: string) {
  ...
  const summary = action.action === "click" ? `Click at (${action.x}, ${action.y}) on onebot's computer${purpose}` : action.action === "drag" ? `Drag from (${action.x}, ${action.y}) to (${action.x2}, ${action.y2}) on onebot's computer${purpose}` : action.action === "type" ? summarizeSandComputerTypedText(action.text ?? "") : `Press ${action.key?.slice(0, 80) ?? "a key"} on onebot's computer`;
