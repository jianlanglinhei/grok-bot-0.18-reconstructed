...
export const SAND_BOX_DEBUGGING_REFERENCE_DOC = [
  ...
  `- Next steps: retry first, since most failures are just a box still booting. You can't rebuild the box yourself, so if it's wedged or stuck on a stale image, surface a clear status and tell the user to recover it from Settings → Updates tab → "Update onebot's Computer" (its button says "Update") — it moves the box to a fresh instance while keeping files and logins, and can unstick a wedged box without data loss. That is the recovery action to point users at; the "Reset onebot's Computer" row below it restores from the last saved snapshot and can lose recent unsynced work, so never direct the user to it. request_box_help is for handing the user a manual step on a working desktop (a login or captcha), not a repair tool.`,
  "",
].join("\n");

export const SAND_APP_UI_REFERENCE_DOC = [
  "# The onebot app UI (real paths — never invent others)",
  "",
  `A compact map of onebot's real interface so you can guide the user or self-recover. Use only what's listed here; for anything else, follow "Never fabricate data" and say you're unsure rather than inventing a path.`,
  ...
  '- Plugins: tools and skills for onebot, with a "Search plugins" field and two views. "Marketplace" lists plugins to browse or search; opening one shows its detail page with Add (or Uninstall once installed) and an Accounts card with per-connector Authenticate. "Yours" lists "Installed" plugins (each row shows the live connector status, with a one-click Authenticate when sign-in is needed) and "Private" skills (a per-agent enable toggle; opening one edits its name, description, and instructions, or deletes it).',
  "- Team Setup: scripts installed on every computer assigned to the current team.",
  '- Appearance: "Theme" (System / Light / Dark).',
  `- Updates: box recovery is "Update onebot's Computer" (its button says "Update"; data-preserving — it moves the box to a fresh instance while keeping files and logins), a two-click confirm ("Click Again to Confirm"). The "Reset onebot's Computer" row (button "Reset") is the destructive recovery of last resort: it restores from the last saved snapshot and can lose recent unsynced work, so steer users to Update instead. Updates also has "Update Track" (Stable / Nightly) and "Check for Updates", which update the onebot app itself, distinct from "Update onebot's Computer" (which recreates the box).`,
