...
function locationForSurface(surface: AutoReviewSurface): string | undefined {
  ...
  if (surface === "box_shell" || surface === "computer") return "Runs on onebot's computer";
  return undefined;
}
...
function AutoReviewApprovalBody({ approval, entryId, isStale }: { approval: AutoReviewApproval; entryId: string; isStale: boolean }) {
  ...
  const hideSummary = approval.command === undefined || approval.summary === "Run a command on your local computer" || approval.summary === "Run a command on onebot's computer" || approval.summary === "Run a command on the agent's VM" || /^Run [“"]/.test(approval.summary) || /^Use .+ tool .+ with /.test(approval.summary);
