...
export function GeneralSettingsPanel({ account, accountPending = false, accountError = null, theme, onAccountAction, onThemeChange, timeZone, localToolPermission, securityKey, autoReview, platform }: GeneralSettingsPanelProps) {
  ...
  const detail = signedIn ? account.email ?? "Signed in to Cursor" : account.kind === "logging-in" ? "Finish signing in from your browser" : "Connect your Cursor account to onebot";
...
export function SecurityKeySettingsGroup({ enabled, platform, onChange }: SecurityKeySettingsGroupProps) {
  ...
  const description = supported
    ? "Allow onebot to use a security key (such as a YubiKey) connected to your computer. You’ll be asked to approve each use."
    : "Security keys from onebot's computer aren't supported on this platform yet.";
  const handleChange = () => {
...
const CANCEL_TRIAL_COPY = "This ends your onebot trial now and removes your remaining trial credits. Your card won’t be charged either way — the trial never turns into a paid plan on its own.";
...
export function UsageSettingsPanel({ meters = [], state, onRetry, onUpgrade, onCancelTrial, onCancelDialogOpen, provider = "cursor" }: UsageSettingsPanelProps) {
  ...
  const upgradeSupportingText = summary == null ? null : upgrade == null ? null
    : !summary.hasNonZeroIncludedLimit && summary.hasAvailableUsage && summary.sandUsagePercent != null && summary.sandUsagePercent < 100
      ? "Get more onebot usage"
      : summary.isSandTrial
        ? "You’ve used all of your trial usage"
        : summary.hasEndedSandTrial
          ? "Your trial has ended. Upgrade to continue using onebot."
          : null;
      ...
      <div className="sand-settings-beta-stack">
        <SettingsGroup title="Updates">
          <div className="sand-settings-beta__status" role="status">
            <span>onebot couldn&apos;t load update status. Check again to retry.</span>
        ...
        {egressTunnel?.featureGateEnabled === true || egressTunnel?.enabled === true ? <EgressTunnelSettingsGroup
          ...
          description={egressTunnel.enabled
            ...
            : egressTunnel.available
              ? "Route web traffic from onebot's computer out through this desktop instead of the cloud. Applies to new connections."
              : "onebot's computer wasn't provisioned with the egress tunnel — start a new one to use this."}
  ...
  return (
    <div className="sand-settings-beta-stack">
      <SettingsGroup title="Updates">
        ...
        <div className="sand-settings-row">
          <span className="sand-settings-copy"><strong>onebot {status.currentVersion}</strong><small>Updates follow the {UPDATE_TRACK_LABELS[status.currentTrack]} track</small></span>
