...
export function disabledUpdateMessage(status: UpdateStatus): string {
  ...
  switch (status.state.reason) {
    ...
    case "lab-build":
      return "onebot Lab is a one-off test build and never auto-updates";
...
export function updateStatusMessage(status: UpdateStatus): UpdateStatusMessage {
  ...
  switch (state.type) {
    ...
    case "available":
      return { text: `onebot ${state.version} is available`, tone: "default" };
    case "downloading": {
      const progress = state.progress != null ? ` (${Math.round(state.progress * 100)}%)` : "";
      return { text: `Downloading onebot ${state.version}…${progress}`, tone: "default" };
    }
    case "staging":
      return { text: `Preparing onebot ${state.version}…`, tone: "default" };
    case "ready":
      return state.lastCheck?.result === "error"
        ? { text: `Update check failed: ${state.lastCheck.errorMessage ?? "unknown error"}. onebot ${state.version} is still ready. Restart to apply.`, tone: "error" }
        : { text: `onebot ${state.version} is ready. Restart to apply.`, tone: "ready" };
    case "idle":
  ...
  switch (status.state) {
    case "connected":
      return status.activeStreams > 0
        ...
        : `Connected — this desktop is ready to route web traffic from onebot's computer (${status.relayedStreams} routed this session).`;
    case "connecting":
      return "Connecting to onebot's computer…";
    case "off":
      return "Enabled, but not routing yet — waiting for onebot's computer to connect with egress enabled.";
  }
}
