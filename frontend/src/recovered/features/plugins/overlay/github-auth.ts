...
export const PLUGIN_AUTH_AGENT_DESCRIPTION = "Sets up git credentials on onebot's computer so installed plugins can be fetched.";
...
export function pluginAuthBlockedDetail(blocks: readonly PluginAuthBlock[]): string {
  ...
  if (names.length === 0 || names.length > 3) {
    ...
    return `${count} installed ${count === 1 ? "plugin" : "plugins"} can't be fetched until onebot's computer can read their source repository.`;
  }
  return `${names.join(", ")} ${names.length === 1 ? "is" : "are"} installed, but their content can't be fetched until onebot's computer can read the source repository.`;
}
