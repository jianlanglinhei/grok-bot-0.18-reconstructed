...
const UPDATE_UNTRACKABLE_COPY = "The computer update started, but onebot can't track its progress. Restart onebot after the computer is available again.";
...
function workingDescription(names: readonly string[]): string {
  if (names.length > 1) {
    return `${formatWorkingAgents(names)} are working on onebot's computer right now. Updating recreates the computer and interrupts their current turns. Files and logins are kept.`;
  }
...
export function projectComputerUpdateConfirmationContent(
  ...
  if (action === "ready") {
    return {
      title: "Update onebot's Computer?",
      ...
      confirmLabel: "Update onebot's Computer",
      cancelLabel: "Not now",
      width: "wide"
    };
  }
