...
const UPDATE_STEPS = ["Getting ready", "Backing up your data", "Recreating onebot's computer", "Starting onebot's computer", "Cleaning up", "Reconnecting"] as const;
const RESET_STEPS = ["Getting ready", "Wiping your data", "Creating onebot's computer", "Starting onebot's computer", "Cleaning up", "Reconnecting"] as const;
const RECOVER_STEPS = ["Getting ready", "Recreating onebot's computer", "Starting onebot's computer", "Reconnecting"] as const;
...
export function projectComputerReconnect(input: Pick<ComputerRebuildBannerInput, "kind" | "stage"> & { readonly transport: "connected" | "down" }): ComputerReconnectProjection | null {
  ...
  if (typeof input.stage === "string" && RECONNECTING_STAGES.has(input.stage)) {
    return { variant: "restarting", title: "onebot's computer restarting", subtitle: "Starting onebot's computer" };
  }
...
export function ComputerRebuildProgressBanner({ input, onRestore }: { readonly input: ComputerRebuildBannerInput; readonly onRestore: () => void }) {
  ...
  const title = input.kind === "reset" ? "Resetting onebot's Computer" : input.kind === "recover" ? "Recovering onebot's Computer" : "Updating onebot's Computer";
  const subtitle = projection.steps[projection.activeIndex]?.label;
  const button = <button aria-label="View onebot's Computer progress" className="sand-computer-rebuild-banner__restore sand-67bb7w sand-3nfvp2 sand-1717udv sand-c342km sand-jbqb8w sand-1q4ynmn sand-1ypdohk" onClick={onRestore} type="button"><ProgressBanner subtitle={subtitle} title={title} value={projection.progress} /></button>;
